...
//        NUMBER_MAPPING.put("9", "玖");
    ...
    public static String getChineseNumber(String number, String unit, String postfix) {
        ...
        String[] numbers = strToStrArray(number);
        if (numbers.length > 2) {
...
//            result.append(postfix == null ? "整" : result.append(postfix));
    ...
    private static String[] strToStrArray (String abc) {
        String[] IDList = abc.split("\\"+DEFAULT_PATH_SEPARATOR);
//        for (int i =0;i<abc.length();i++){
//            IDList[i] = abc.substring(i,i+1);
//        }
        return IDList;
    }
}
